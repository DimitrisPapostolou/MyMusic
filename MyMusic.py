...
class SpotifyClone(QWidget):
    ...
    def refresh_home(self):
        ...
        for playlist_name, songs in self.playlists.items():
            ...
            for i, song_path in enumerate(songs[:4]):
                art_label = QLabel()
                art_label.setFixedSize(64,64)
                art_label.setStyleSheet("border: 1px solid #333; background-color: #444;")
                try:
                    audio = ID3(song_path)
                    for tag in audio.values():
                        if isinstance(tag, APIC):
                            ...
                            art_label.setPixmap(pix.scaled(64,64,Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation))
                            break
                except: pass
                art_grid.addWidget(art_label, i//2, i%2)
            layout.addLayout(art_grid)
        ...
        recent_songs = sorted(self.library_songs, key=os.path.getmtime, reverse=True)[:8]
        col, row = 0, 0
        for song_path in recent_songs:
            frame = QFrame()
    ...
    def show_playlist_songs(self, item):
        ...
        for song in self.current_song_list:
            song_item = QListWidgetItem(os.path.basename(song))
            try:
                audio = ID3(song)
                for tag in audio.values():
                    if isinstance(tag, APIC):
                        ...
                        pix = QPixmap()
                        pix.loadFromData(QByteArray(data))
                        song_item.setIcon(QIcon(pix))
                        break
            except: pass
            self.playlist_songs.addItem(song_item)
