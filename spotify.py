...
PLACEHOLDER_ART = "placeholder.png"  # Put a placeholder image in your project folder
...
class SpotifyClone(QWidget):
    def __init__(self):
        ...
        def refresh_home():
            ...
            # --- Show Playlists with Mini Album Grid ---
            col = 0
            row = 0
            for playlist_name, songs in self.playlists.items():
                ...
                # Mini grid for album arts (2x2)
                mini_grid = QGridLayout()
                mini_grid.setSpacing(2)
                for i in range(4):
                    icon_label = QLabel()
                    icon_label.setFixedSize(60, 60)
                    if i < len(songs):
                        pix = self.get_album_art(songs[i], 60)
                        if pix:
                            icon_label.setPixmap(pix)
                        else:
                            icon_label.setStyleSheet("background-color: #333; border-radius: 4px;")
                    else:
                        icon_label.setStyleSheet("background-color: #333; border-radius: 4px;")
                    mini_grid.addWidget(icon_label, i//2, i%2)
                frame_layout.addLayout(mini_grid)
                ...
                frame_layout.addWidget(play_btn, alignment=Qt.AlignmentFlag.AlignCenter)

                # Hover events
                def enterEventFactory(btn):
                    def enter(event):
                        btn.setVisible(True)
                    return enter
                def leaveEventFactory(btn):
                    def leave(event):
                        btn.setVisible(False)
                    return leave
                frame.enterEvent = enterEventFactory(play_btn)
                frame.leaveEvent = leaveEventFactory(play_btn)
            ...
            for song_path in self.library_songs[:8]:
                ...
                pix = self.get_album_art(song_path, 80)
                if pix:
                    icon_label.setPixmap(pix)
                else:
                    icon_label.setStyleSheet("background-color: #333; border-radius: 4px;")
        ...
        original_load_playlists = self.load_playlists
        self.load_playlists = lambda: (original_load_playlists(), refresh_home())
        self.load_playlists()
    ...
    def load_playlists(self):
        if os.path.exists(DATA_FILE):
            ...
            for song in self.library_songs:
                self.library_list.addItem(QListWidgetItem(song))
    ...
    def add_songs(self):
        paths, _ = QFileDialog.getOpenFileNames(
            self, "Add audio files", "", "Audio Files (*.mp3 *.wav *.flac);;All Files (*)"
        )
        for p in paths:
            if p not in self.library_songs:
                self.library_songs.append(p)
                self.library_list.addItem(QListWidgetItem(p))
        self.filter_songs()
        self.save_playlists()
        # Refresh Home after adding songs
        self.load_playlists()
    ...
    def filter_songs(self):
        ...
        for song in self.library_songs:
            if query in song.lower():
                self.search_results.addItem(song)
    ...
    def add_song_to_playlist(self, song_path):
        ...
        if ok and name:
            if song_path not in self.playlists[name]:
                ...
                if self.playlist_list.currentItem() and self.playlist_list.currentItem().text() == name:
                    item = QListWidgetItem(song_path)
                    pix = self.get_album_art(song_path, 32)
                    if pix:
    ...
    def show_playlist_songs(self, item):
        ...
        for song in self.current_song_list:
            ...
            pix = self.get_album_art(song, 32)
            if pix:
                item.setIcon(QIcon(pix))
            self.playlist_songs.addItem(item)
    ...
    def play_selected_song(self, item):
        song = item.text()
        if song in self.current_song_list:
            self.current_index = self.current_song_list.index(song)
        else:
            self.current_song_list = [song]
            self.current_index = 0
        self.play_current_song()
    ...
    def play_current_song(self):
        if 0 <= self.current_index < len(self.current_song_list):
            ...
            self.player.play()
            pix = self.get_album_art(song_path, 64)
            if pix:
                self.album_art.setPixmap(pix)

    # ===== ALBUM ART EXTRACTION =====
    def get_album_art(self, filepath, size):
        ...
        if os.path.exists(PLACEHOLDER_ART):
            return QPixmap(PLACEHOLDER_ART).scaled(size, size, Qt.AspectRatioMode.KeepAspectRatio)
        return None
